...
mighty.CreateModule("Entity", "Editor-TERRAIN",
	{
		activate: function()
		{
			var self = this;
		...
		doMode_ADD: function()
		{
			...
			var cursor = Cursor.plugin;
			if(cursor.isInvalid) { return; }

			var terrainCfg = Terrain.Cfg;
			var gridX = Math.floor(cursor.x / terrainCfg.tileWidth);
			var gridY = Math.floor(cursor.y / terrainCfg.tileHeight);
			var index = gridX + (gridY * terrainCfg.numTilesX);
			console.log(index, gridX, gridY);

			Entity.plugin.setGridInstance(index, this.entity);
		...
		handleSignal_EntityInteract: function(event, entity)
		{
			switch(event)
			{
				...
				{
					if(entity && this.overEntity !== entity) {
		...
		useItemID: function(itemID)
		{
			...
			this.entity = mighty.Macro.CreateEntity(Palettes.Entity.getByID(itemID).name);
