function DrawAABB(minX, minY, maxX, maxY)
{
	this.minX = minX;
	this.minY = minY;
	this.maxX = maxX;
	...
	this.x = this.minX + (this.sizeX / 2);
	this.y = this.minY + (this.sizeY / 2);
}

DrawAABB.prototype =
{
	translate: function(posX, posY)
	{
		this.minX += posX;
		this.minY += posY;
		this.maxX += posX;
	...
	move: function(x, y)
	{
		this.minX = x;
		this.minY = y;
		this.maxX = x + this.sizeX;
		this.maxY = y + this.sizeY;
	},
	...
	addDiff: function(diffX, diffY)
	{
		this.minX -= diffX;
		this.minY -= diffY;

		this.sizeX += (diffX + diffX);
		this.sizeY += (diffY + diffY);
